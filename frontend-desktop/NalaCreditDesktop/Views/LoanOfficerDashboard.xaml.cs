...
namespace NalaCreditDesktop.Views
{
    public partial class LoanOfficerDashboard : Window
    {
        ...
        public void ApproveApplication_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.Tag is Guid applicationId)
            {
                _ = ApproveApplicationAsync(applicationId);
            }
        }

        public void RejectApplication_Click(object sender, RoutedEventArgs e)
        {
            if (sender is Button button && button.Tag is Guid applicationId)
            {
                _ = RejectApplicationAsync(applicationId);
            }
        }

        private async Task ApproveApplicationAsync(Guid applicationId)
        {
            try
            {
                // Get application details
                var appResult = await _apiService.GetMicrocreditApplicationAsync(applicationId);
                
                if (!appResult.IsSuccess || appResult.Data == null)
                {
                    MessageBox.Show("Erreur: Impossible de charger les détails de la demande",
                        "Erreur", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                var dialog = new ApproveApplicationDialog(
                    application.ApplicationNumber,
                    application.RequestedAmount,
                    application.MonthlyInterestRate,
                    application.RequestedDurationMonths);

                if (dialog.ShowDialog() == true)
                {
                    // Prepare approval data
                    var approvalData = new Models.ApproveMicrocreditApplicationDto
                    {
                        ApprovedAmount = dialog.ApprovedAmount,
                        InterestRate = dialog.InterestRate,
                        DurationMonths = dialog.DurationMonths,
                        DisbursementDate = dialog.DisbursementDate,
                        Comments = dialog.Comments
                    };

                    // Call API
                    var result = await _apiService.ApproveMicrocreditApplicationAsync(applicationId, approvalData);

                    if (result.IsSuccess)
                    {
                        MessageBox.Show($"Demande approuvée avec succès!\n\n" +
                            $"Montant: {dialog.ApprovedAmount:N2} HTG\n" +
                            $"Taux: {dialog.InterestRate * 100:N2}%\n" +
                            $"Durée: {dialog.DurationMonths} mois\n" +
                            $"Décaissement: {dialog.DisbursementDate:dd/MM/yyyy}",
                    ...
                    {
                        MessageBox.Show($"Erreur lors de l'approbation:\n{result.ErrorMessage}",
                            "Erreur", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
        ...
        private async Task RejectApplicationAsync(Guid applicationId)
        {
            try
            {
                // Get application details
                ...
                var dialog = new RejectApplicationDialog(application.ApplicationNumber);

                if (dialog.ShowDialog() == true)
                {
                    // Prepare rejection data
                    var rejectionData = new Models.RejectMicrocreditApplicationDto
                    {
                        RejectionReason = dialog.RejectionReason,
                        Comments = dialog.Comments
                    };

                    // Call API
                    var result = await _apiService.RejectMicrocreditApplicationAsync(applicationId, rejectionData);

                    if (result.IsSuccess)
                    {
                        MessageBox.Show($"Demande rejetée.\n\n" +
                            $"Raison: {dialog.RejectionReason}",
                            "Demande Rejetée", MessageBoxButton.OK, MessageBoxImage.Information);
                    ...
                    {
                        MessageBox.Show($"Erreur lors du rejet:\n{result.ErrorMessage}",
                            "Erreur", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
