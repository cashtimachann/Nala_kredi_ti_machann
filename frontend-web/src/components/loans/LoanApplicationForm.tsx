...
const LoanApplicationForm: React.FC<LoanApplicationFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const collateralValidation = useMemo(() => {
    ...
    if (collateralType === 'Épargne bloquée') {
      const minimumRequired = requestedAmount * 0.15;
      if (savingsBalance < minimumRequired) {
        return {
          valid: false,
          message: `⚠️ Solde d'épargne insuffisant. Vous avez ${formatCurrency(savingsBalance, currency)} mais il faut au minimum ${formatCurrency(minimumRequired, currency)} (15% du montant demandé)`
        };
      }
