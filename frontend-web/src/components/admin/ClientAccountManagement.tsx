...
const AccountCreationForm: React.FC<AccountCreationFormProps> = ({ onSubmit, onCancel }) => {
  ...
  // Existing accounts for the selected customer
  const [existingAccounts, setExistingAccounts] = useState<ClientAccount[]>([]);
  const [loadingExistingAccounts, setLoadingExistingAccounts] = useState<boolean>(false);
  ...
  // Fetch existing accounts when customer is selected
  useEffect(() => {
    ...
    (async () => {
      try {
        // Fetch all accounts for this customer (we'll filter by account type and currency on the frontend)
        ...
        setExistingAccounts(customerAccounts);
      } catch (error) {
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {(['HTG', 'USD'] as const).map((curr) => {
            // Check if customer already has an account with this currency for the selected account type
            const hasExistingAccount = existingAccounts.some(account =>
              account.accountType === accountType && account.currency === curr
            );
            ...
            return (
              <button
                key={curr}
                type="button"
                onClick={() => !hasExistingAccount && setCurrency(curr)}
                ...
                title={hasExistingAccount ? `Le client possède déjà un compte ${getAccountTypeLabel(accountType).toLowerCase()} en ${curr}` : undefined}
              >
...
const ClientAccountManagement: React.FC<ClientAccountManagementProps> = () => {
  ...
  toast.success(`Compte créé avec succès • Numéro: ${newAccount?.accountNumber || '—'}`);
      ...
      if (status === 403) {
        ...
        toast.error(`Conflit: ${extracted || 'Le client possède déjà un compte avec cette devise pour ce type de compte.'}`);
      } else if (status === 400) {
